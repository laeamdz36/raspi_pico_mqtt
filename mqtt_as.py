...
class MQTT_base:
    ...
    async def _connect(self, clean):
        ...
        if connack_props_length > 0:
            ...
            decoded_props = decode_properties(
                connack_props, connack_props_length)
    ...
    async def wait_msg(self):
        ...
        if op == 0x90 or op == 0xB0:  # [UN]SUBACK
            ...
            if suback or mqttv5:
                ...
                if reason_code >= 0x80:
                    raise OSError(-1,
                                  f"{un}SUBACK reason code 0x{reason_code:x}")
        ...
        if op == 0xE0:  # DISCONNECT
            if mqttv5:
                ...
                if sz > 0:
                    ...
                    decoded_props = decode_properties(
                        disconnect_props, dis_props_sz)
                    self.dprint("DISCONNECT properties %s", decoded_props)

                if reason_code >= 0x80:
                    raise OSError(-1, "DISCONNECT reason code 0x%x" %
                                  reason_code)
